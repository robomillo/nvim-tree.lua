...
M.View = {
    ...
    winopts = {
        ...
        winhl = table.concat({
            ...
            "SignColumn:NvimTreeSignColumn", "NormalNC:NvimTreeNormalNC"
        }, ",")
    }
}
...
local function wipe_rogue_buffer()
    for _, bufnr in ipairs(a.nvim_list_bufs()) do
        if not matches_bufnr(bufnr)
            and a.nvim_buf_get_name(bufnr):match "NvimTree" ~= nil then
...
local function open_window()
    ...
    M.View.tabpages[tabpage] = vim.tbl_extend("force", M.View.tabpages[tabpage]
                                                  or {help = false},
                                              {winnr = winnr})
end
